...
def f(b,*c):
    ...
    return [- k_dE * L * C_d * C_E + lambda_D * C_DD , 
            lambda_D * C_DD -  k_D * C_DT - k_dD * L * C_d * C_DT,
            -k_dE * L * C_d * C_E + k_de * C_de,       
...
while negative_eigenvalue:
    ...
    for i in range(N+1):
        ...
        A[1,0] = lambda_D
        A[1,1] = - k_D - k_dD * L * C_d - D_D * q * q
        A[1,3] = - k_dD * L * C_DT
        
        A[2,2] = - k_dE * L * C_d - D_E * q * q
        A[2,3] = - k_dE * L * C_E
        A[2,4] = k_de
        
        A[3,1] = k_D + k_dD * L * C_d
        A[3,2] = -k_dE * L * C_d 
        A[3,3] = k_dD * L * C_DT - k_dE * L * C_E - D_m * q * q
...
Nt = int(tFinal/dt)
tF = dt*Nt
xspan = np.linspace(x0, xL, Nx, dtype=float)
...
# ---initial condition
DT[0:int(Nx/2),0] = N_DD / int(Nx/2)
E[:,0] = N_E / Nx

for k in range(Nt):
    #---dx/dt---
    f1 = -lambda_D * DD[:,k] + k_de * de[:,k]
    f2 =  lambda_D * DD[:,k] - k_D * DT[:,k] - k_dD * DT[:,k] * d[:,k] * Nx
    f3 =  - k_dE * d[:,k] * E[:,k] * Nx + k_de * de[:,k]
    f4 = k_D * DT[:,k] + k_dD * DT[:,k] * d[:,k] * Nx - k_dE * d[:,k] * E[:,k] * Nx
    f5 = k_dE * d[:,k] * E[:,k] * Nx - k_de * de[:,k]
